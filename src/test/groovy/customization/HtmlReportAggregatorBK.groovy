...
class HtmlReportAggregatorBK extends AbstractHtmlCreator<Map> {
    ...
    protected HtmlReportAggregatorBK() {
    ...
    void aggregateReport(SpecData data, Map stats) {
        ...
        aggregatedData[specName] = Utils.createAggregatedData(allFeatures[false], allFeatures[true], stats, specTitle, narrative)
    }
    ...
    protected void writeSummary(MarkupBuilder builder, Map json) {
        ...
        def cssClassIfTrue = { isTrue, String cssClass -> if (isTrue) ['class': cssClass] else Collections.emptyMap()
        }
        ...
        builder.div('class': 'summary-report') {
            ...
            table('class': 'summary-table') {
                ...
                tbody {
                    tr {
                        td stats.total
                        def totalExecuted = countFeatures( stats.featureRuns ) { FeatureRun run -> !isSkipped( run.feature ) }
    ...
    protected void writeFooter(MarkupBuilder builder) {
        builder.div('class': 'footer') {
            mkp.yieldUnescaped("Generated by zidingyi")
        }
    }
